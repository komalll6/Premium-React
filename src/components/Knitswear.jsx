...
import knitswearData from '../data/knitswearData'; // Fixed file spelling typo

function Knitswear() {
  // Fixed fallback checking mechanism to use knitswearData instead of shirtsData
  const cleanData = Array.isArray(knitswearData) 
    ? knitswearData 
    : (knitswearData && Array.isArray(knitswearData.knitswearData) ? knitswearData.knitswearData : []); 

  if (cleanData.length === 0) {
    return (
      <div className="pt-32 text-center min-h-screen bg-white text-zinc-800">
        <p className="tracking-widest uppercase text-sm">No items matching knitswear specifications found.</p>
      </div>
    );
  }
