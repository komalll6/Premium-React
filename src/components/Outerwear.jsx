...
import outerwearData from '../data/outerwearData'; // Fixed file spelling typo

function Outerwear() {
  // Fixed fallback checking mechanism to use outerwearData instead of shirtsData
  const cleanData = Array.isArray(outerwearData) 
    ? outerwearData 
    : (outerwearData && Array.isArray(outerwearData.outerwearData) ? outerwearData.outerwearData : []); 

  if (cleanData.length === 0) {
    return (
      <div className="pt-32 text-center min-h-screen bg-white text-zinc-800">
        <p className="tracking-widest uppercase text-sm">No items matching outerwear specifications found.</p>
      </div>
    );
  }
      ...
      <div style={{ maxWidth: '1400px' }} className="mx-auto px-12 lg:px-20 -mt-20">        
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-10 -mb-20">
          {cleanData.map((item) => (
            ...
            className="font-normal tracking-widest text-zinc-500 uppercase leading-relaxed mb-1">                {item.name}
              </h3>

              {/* Price Tag */}
            <span style={{ fontSize: '13px' }} className="font-bold tracking-wider text-zinc-900">                {item.price}
              </span>
