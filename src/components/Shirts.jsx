...
function Shirts() {
  ...
  if (cleanData.length === 0) {
    return (
      <div className="pt-32 text-center min-h-screen bg-white text-zinc-800">
        <p className="tracking-widest uppercase text-sm px-4">No items matching shirt specifications found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="bg-white min-h-screen pt-8 pb-14 text-zinc-900 font-sans">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-8 md:px-12 lg:px-20">        
        ...
        {/* Left-Aligned Clean Section Header */}
        <div className=" pl-2">
          <h1 className="text-lg sm:text-xl md:text-[22px] font-bold tracking-[0.18em] uppercase text-zinc-900 ">
            Business Suiting Designs
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-8">
          {cleanData.map((shirt) => (
            <div key={shirt.id} className="flex flex-col items-center text-center group cursor-pointer w-full">
              
              {/* Image Frame */}
              <div className="w-full h-72 sm:h-80 md:h-96 bg-white overflow-hidden flex items-center justify-center relative">
                <img
                  src={shirt.image}
                  alt={shirt.name}
                  className="w-full h-full object-contain object-center mix-blend-multiply transition-transform duration-500 ease-out group-hover:scale-102"
